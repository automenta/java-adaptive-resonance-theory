...
public abstract class MineFieldSimulator {
    ...
    private static final Logger logger = Logger.getLogger(String.valueOf(MineFieldSimulator.class));
    ...
    public boolean senseActSense(int agentId, boolean last, boolean provideImmediateReward) {
        ...
        if (v != -1) {  // if valid move
            if (last && !provideImmediateReward)  //run out of time (without immediate reward)
                r = 0.0;
            else
